...
//                }
    ...
    //判断该请求所请求的数据块是否已缓存
    long long reqCurrentOffset = loadingRequest.dataRequest.currentOffset;
    long long reqRequireLen    = loadingRequest.dataRequest.requestedLength;
    
    NSInteger startSegment  = [MTAVResourceInfo segmentWithOffset:reqCurrentOffset];
    NSInteger endSegment    = [MTAVResourceInfo segmentWithOffset:reqCurrentOffset Length:reqRequireLen];
    
    NSLog(@"loadrequest: reqOffset = %lld, reqLength = %lld, startSegment = %ld, endSegment = %ld",
        ...
        case MTAVResourceInfoStatus_Partial:
        {
            ...
            if (isCached) {
                ...
                for (MTAVResourceDataBlock *block in blockArr) {
                    NSLog(@"bock.startSegment = %ld, endSegment = %ld", block.startSegment, block.endSegment);
                }
...
- (void)processPendingRequests {
    ...
    NSLog(@"pendingRequestArr.count = %ld", self.pendingRequestArr.count);
...
- (void)fillContentInformation:(AVAssetResourceLoadingContentInformationRequest *)contentInformationRequest {
    ...
    long long contentLen  = self.loaderTask.curResourceInfo.contentLength;
...
- (void)responseWholeDataForRequest:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    long long reqCurrentOffset  = loadingRequest.dataRequest.currentOffset;
    long long reqRequireLen     = loadingRequest.dataRequest.requestedLength;
    NSRange range               = NSMakeRange(reqCurrentOffset, reqRequireLen);
...
- (BOOL)respondWithDataForRequest:(AVAssetResourceLoadingDataRequest *)dataRequest {
    ...
    //判断该请求所请求的数据块是否已缓存
    long long reqRequireLen     = dataRequest.requestedLength;
    long long reqCurrentOffset  = dataRequest.currentOffset;
    ...
    if (isCached) {
        ...
        if (lastSegment >= startSegment) {
            ...
            NSRange range = NSMakeRange(reqCurrentOffset, lastOffset - reqCurrentOffset);

            NSLog(@"response___loadingRequest: currentOffset = %ld, len = %ld", range.location, range.length);
            [dataRequest respondWithData:[filedata subdataWithRange:range]];
            didRespondFully = NO;
